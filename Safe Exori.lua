...
local MANA = 20						    -- min mana need to cast spell
local FRIENDS = {"Friend1", "Friend2"} 	-- list of friends we can use exori Capital letters like a Character Name
local MONSTERS = {"Demon", "Dragon"}     -- monsters list to attack.
local MONSTERS_AMOUNT_TO_USE = 1		-- min monsters amount to cast spell
...
local FRIENDS = table.lower(FRIENDS)
local MONSTERS = table.lower(MONSTERS)

----------------------------------------------------------------------------------------------------------------------------------------------------------
--> Function:		ableToCastExori(friends, monstersAmount, monsters, playerRange)
...
-->                 @monsters table with creature names
...
function ableToCastExori(friends, monstersAmount, monsters, playerRange)
	...
	for i = 1, #creatures do
        ...
        -- if creature is monster count amount and is valid name.   
...
Module.New("Safe Exori", function ()
	...
	if Self.Mana() >= MANA and Self.HealthPercent() >= SELF_MIN_HPPERC then
		...
		if os.time() - useSpellTime >= 2 then
			...
			if ableToCastExori(FRIENDS, MONSTERS_AMOUNT_TO_USE, MONSTERS, DONT_CAST_WHEN_PLAYER_DIST) then
