...
function getPlayer(pos)
	...
	for i, player in pairs(Creature.getCreatures()) do
		...
		if not table.find(FRIENDS, string.lower(player.name)) and Creature.isPlayer(player) and player.name ~= selfName then
...
Module.New("Multifloor Player Step", function (mod)
